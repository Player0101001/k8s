...
带有确定完成计数的 Job，即 .spec.completions 不为 null 的 Job， 都可以在其 .spec.completionMode 中设置完成模式：job结束需要成功运行pod个数，即状态为completed的pod数。
...
[root@kmaster job]# kubectl get pod  --开始创建pod
NAME                    READY   STATUS              RESTARTS   AGE
